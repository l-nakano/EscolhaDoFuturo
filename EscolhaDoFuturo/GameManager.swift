...
class GameManager {
    ...
    let textsList = [
        "João tem 17 anos e irá morar sozinho pela primeira vez. Desde de criança ele guardava um pouco de dinheiro para comprar seus álbuns de figurinhas preferidos.",
        ...
        "Sempre antes de tomar uma decisão, João acessava o app de seu banco para verificar o saldo disponível e fazia as contas para saber quanto sobraria ao final de cada mês.",
        "Desde cedo seu pai depositava dinheiro em uma poupança, para que João pudesse utilizar quando mais velho.",
    ...
    let finalSceneList: () -> String = {
        ...
        if shared.hasMoto {
            if shared.FV > 0 {
                return "Como João tinha poupado, conseguiu pagar o tratamento e teve dinheiro para os gastos mensais até encontrar um novo emprego, até mesmo passar por outro imprevisto. Ainda poderia continuar pagando o financiamento da sua moto."
            } else {
            ...
            if shared.FV > 0 {
                return "Como João tinha poupado, conseguiu pagar o tratamento e teve dinheiro para os gastos mensais até encontrar um novo emprego e até mesmo passar por outro imprevisto."
            } else {
    ...
    let decisionsList = [
        ...
        6: Decision(choicesQty: 2, choicesLabel: ["Compraria o computador", "Pouparia o dinheiro"], question: "O que você faria no lugar de João?", answers: [[PerfisInvestidor.nenhum, Float(-7000), Float(0), false], [PerfisInvestidor.nenhum, Float(0), Float(0), false]]),
        8: Decision(choicesQty: 2, choicesLabel: ["Sairia com os amigos", "Guardaria a mesada"], question: "Qual decisão você tomaria no lugar de João?", answers: [[PerfisInvestidor.nenhum, Float(0), Float(0), false], [PerfisInvestidor.nenhum, Float(0), Float(500), false]]),
